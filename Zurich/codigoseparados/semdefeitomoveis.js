...
async carregaOS(){
  const response = await this.acaoModel.manualQuery({
    ...
    query: "SELECT IZ.id, IZ.Sinistro, IZ.OS, IZ.valorNF, IZ.orcamento, IZ.reincidencia, IZ.uf, (0.00) as PagoNoPeriodo, (0.00) as PagoAnteriorAoPeriodo, IZ.Desconto, IZ.ValorAprovado, IZ.id_emp, STR_TO_DATE(IZ.data, '%d/%m/%Y') as data, STR_TO_DATE(D.desc_data, '%d/%m/%Y') as dataDesc FROM importados_zurich IZ LEFT JOIN descricoes D ON D.desc_id_zurich = IZ.id LEFT JOIN selects S ON S.sel_id = IZ.status LEFT JOIN empresas E ON E.id = IZ.id_emp LEFT JOIN cidades C ON C.id = IZ.cidade LEFT JOIN lancamento_pg LP ON LP.idOS = IZ.id WHERE ( (IZ.id_emp IN (1) AND S.sel_id IN (78)) ) AND ((LP.LiberadoPg != 0 AND LP.statusPG = 19 OR LP.LiberadoPg != 0 AND LP.statusPG = 20 AND LP.LiberadoNPS != 0) OR LP.id is null) AND (STR_TO_DATE(D.desc_data, '%d/%m/%Y') >= '2022-08-01' AND STR_TO_DATE(D.desc_data, '%d/%m/%Y') <= '2030-12-23') GROUP BY IZ.id",
