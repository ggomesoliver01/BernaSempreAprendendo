...
class statusreparo{
  ...
  async carregaOS(){
    const response = await this.acaoModel.manualQuery({
      ...
      query: "SELECT IZ.id, IZ.OS, IZ.Sinistro,IZ.id_emp, IZ.data, AG.A_Data as AG, S.sel_nome, (SELECT D.desc_descricao FROM descricoes D where D.desc_id_zurich = IZ.id AND D.desc_descricao LIKE CONCAT('%', S.sel_nome, '%') order by D.desc_id desc limit 1) as 'descricao'FROM importados_zurich IZ JOIN agendamento AG ON AG.A_id_Zurich = IZ.id left join selects S ON S.sel_id = IZ.status WHERE IZ.id_emp IN (64,71,111,146) AND gatilho = 0 AND taxa_flet = 1 AND status = 18 AND (STR_TO_DATE(IZ.data,'%d/%m/%Y') >= '2022-12-01' AND STR_TO_DATE(IZ.data,'%d/%m/%Y') <= '2030-12-30') group by IZ.id;",
    ...
    //           sqlQuery:   "SELECT IZ.id, IZ.OS, IZ.Sinistro,IZ.id_emp, IZ.data, AG.A_Data as AG, S.sel_nome, (SELECT D.desc_descricao FROM descricoes D where D.desc_id_zurich = IZ.id AND D.desc_descricao LIKE CONCAT('%', S.sel_nome, '%') order by D.desc_id desc limit 1) as 'descricao'FROM importados_zurich IZ JOIN agendamento AG ON AG.A_id_Zurich = IZ.id left join selects S ON S.sel_id = IZ.status WHERE IZ.id_emp IN (64,71,111) AND gatilho = 0 AND taxa_flet = 1 AND status = 18 AND (STR_TO_DATE(IZ.data,'%d/%m/%Y') >= '2022-12-01' AND STR_TO_DATE(IZ.data,'%d/%m/%Y') <= '2030-12-30') group by IZ.id;" //AND IZ.OS = '3656032'
    //         },
    //         {  //AND IZ.OS = 'AND IZ.OS = '3904697''
    //           headers:{                                                       
