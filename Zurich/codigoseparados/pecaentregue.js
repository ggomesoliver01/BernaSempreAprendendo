...
class PecaEnviada {
  ...
  async listagem(page, browser, arr = [], i = 0) {
    try {
      ...
      if (AG = ! '' && dataCerta >= AG) {
        ...
        if (nota.indexOf("peças chegaram ao nosso posto de atendimento. Produto se encontra em reparo, prazo máximo para finalização será dia ${pedrao}, com grande possibilidade de antecipação.") !== -1 && nota != ('undefined')) {
  ...
  async carregaOS() {
    const response = await this.acaoModel.manualQuery({
      ...
      WHERE IZ.id_emp IN (64,71,146,1)
        AND IZ.gatilho = 0
        AND taxa_flet = 0
        AND IZ.status = 137
        GROUP BY IZ.id`,
