...
class statusreparo{
 ...
 async importaZurich(i = 0, arr = []) {
    ...
    if(peca == "2"){
      console.log('2')
      await page.waitForTimeout(2000)
      await page.type('#collapseMessage > app-loading > div.content.ng-star-inserted > div > div > div > div > div.type_msg > div > div > input',`Prezados, estaremos entrando em contato com a segurada e alinhando com a mesma a melhor data de recolha do produto. Previsão de finalização de reparo até o dia ${AG}.`)
...
console.log('Gatilho alterado')
  ...
  WHERE IZ.id_emp IN (1)
    AND IZ.gatilho = 0
      ...
      AND IZ.devolucao_peca is not null
  ...
  //         GROUP BY IZ.id` //AND IZ.OS = '3687877'
  //       },
  //       {  //AND IZ.OS = '3838442'
  //         headers:{                                                       
