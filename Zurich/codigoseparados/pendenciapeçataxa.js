 ...
 async importaZurich(i = 0, arr = []) {
  ...
  async carregaOS(){
    const response = await this.acaoModel.manualQuery({
      ...
      query: "SELECT IZ.OS,IZ.id_emp, IZ.id FROM importados_zurich IZ WHERE IZ.id_emp IN (64,71,111,146) AND gatilho = 0  AND taxa_flet = 1 AND status = 16 AND (STR_TO_DATE(IZ.data,'%d/%m/%Y') >= '2023-02-10' AND STR_TO_DATE(IZ.data,'%d/%m/%Y') <= '2030-12-30')  group by IZ.id;",
    ...
    //           sqlQuery:   "SELECT IZ.OS,IZ.id_emp, IZ.id FROM importados_zurich IZ WHERE IZ.id_emp IN (64,71) AND gatilho = 0  AND taxa_flet = 1 AND status = 16 AND (STR_TO_DATE(IZ.data,'%d/%m/%Y') >= '2023-02-10' AND STR_TO_DATE(IZ.data,'%d/%m/%Y') <= '2030-12-30')  group by IZ.id;" //AND IZ.OS = '3713772'
    //         },
    //         {  //AND IZ.OS = '3713772'
    //           headers:{                                                       
