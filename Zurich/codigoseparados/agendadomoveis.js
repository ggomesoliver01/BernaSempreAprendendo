...
class statusagendado{
 ...
 async importaZurich(i = 0, arr = []) {
    ...
    await page.type('#collapseMessage > app-loading > div.content.ng-star-inserted > div > div > div > div > div.type_msg > div > div > input',`Prezados, visita agendada para o dia ${AG}.`)
    await rep.updateGatilhoSinistros(arr[i].id);
    await page.waitForTimeout(3000)
  ...
  async carregaOS(){
    const response = await this.acaoModel.manualQuery({
      ...
      IF(@varAG IS NULL,
      (SELECT A.A_Data FROM agendamento A WHERE A.A_id_Zurich = IZ.id ORDER BY A.A_id DESC limit 1),
      @varAG) as AG,
      AG.status_id,
      IZ.status,
      S.sel_nome,
