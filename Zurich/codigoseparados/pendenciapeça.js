...
class statusnaoenviado{
 ...
 async importaZurich(i = 0, arr = []) {
    ...
    await page.type('#collapseMessage > app-loading > div.content.ng-star-inserted > div > div > div > div > div.type_msg > div > div > input','Prezados, estamos realizando a cotação das peças com o fabricante, em breve informamos previsão de chegada das peças.')
  ...
  async carregaOS(){
    const response = await this.acaoModel.manualQuery({
      ...
      query: "SELECT IZ.OS, IZ.id FROM importados_zurich IZ WHERE IZ.id_emp IN (64,71,146,111,1) AND taxa_flet = 0 AND gatilho = 0  AND CHAR_LENGTH(IZ.OS) < 9 AND status = 16 AND  (STR_TO_DATE(IZ.data,'%d/%m/%Y') >= '2023-02-10' AND STR_TO_DATE(IZ.data,'%d/%m/%Y') <= '2030-12-30')  group by IZ.id;",
    ...
    //           sqlQuery:   "SELECT IZ.OS, IZ.id FROM importados_zurich IZ WHERE IZ.id_emp IN (64,71) AND taxa_flet = 0 AND gatilho = 0 AND status = 16 AND (STR_TO_DATE(IZ.data,'%d/%m/%Y') >= '2023-02-10' AND STR_TO_DATE(IZ.data,'%d/%m/%Y') <= '2030-12-30')  group by IZ.id;" //AND IZ.OS = '3713772'
    //         },
    //         {  //AND IZ.OS = '3713772'
    //           headers:{                                                       
