...
//  if(AG == ""){
  ...
  async carregaOS(){
    const response = await this.acaoModel.manualQuery({
      ...
      query: "SELECT IZ.id,IZ.OS,IZ.Sinistro,IZ.id_emp,IZ.data,S.sel_nome  FROM importados_zurich IZ left join selects S ON S.sel_id = IZ.status  WHERE IZ.id_emp IN (64,71,111,146,1) AND gatilho = 0 AND taxa_flet = 0 AND status in  (12,5)  AND (STR_TO_DATE(IZ.data,'%d/%m/%Y') >= '2023-04-17' AND STR_TO_DATE(IZ.data,'%d/%m/%Y') <= '2030-12-30')  group by IZ.id;",
