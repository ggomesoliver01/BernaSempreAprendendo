 ...
 async importaZurich(i = 0, arr = []) {
  ...
  async carregaOS(){
    const response = await this.acaoModel.manualQuery({
      ...
      query: `SELECT IZ.id,IZ.OS,IZ.Sinistro,IZ.data,S.sel_nome  FROM importados_zurich IZ left join selects S ON S.sel_id = IZ.status  WHERE IZ.id_emp IN (71,111,64,146) AND taxa_flet = 1 AND gatilho = 0 AND status = 167  AND (STR_TO_DATE(IZ.data,'%d/%m/%Y') >= '2023-04-17' AND STR_TO_DATE(IZ.data,'%d/%m/%Y') <= '2030-12-30')  group by IZ.id;`,
  ...
  //         sqlQuery: `SELECT IZ.id,IZ.OS,IZ.Sinistro,IZ.data,S.sel_nome  FROM importados_zurich IZ left join selects S ON S.sel_id = IZ.status  WHERE IZ.id_emp IN (71,111,64) AND taxa_flet = 1 AND gatilho = 0 AND status = 167  AND (STR_TO_DATE(IZ.data,'%d/%m/%Y') >= '2023-04-17' AND STR_TO_DATE(IZ.data,'%d/%m/%Y') <= '2030-12-30')  group by IZ.id;` //AND IZ.OS = '3687877'
