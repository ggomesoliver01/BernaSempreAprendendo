...
class importacaomoveis {
  ...
  importaZurich(dadosTerminal) {
    ...
    (async () => {
      ...
      await page.waitForTimeout(10000)
           ...
           // DOWNLOAD
            await page.goto('http://zews.zurich.com.br/PortalPrestador/service-orders?service=2&periodCreated=1')
  ...
  carregaExcel(dadosTerminal){
    ...
    xlData.forEach(element => {
      ...
      Object.keys(element).forEach(function(key) {
       ...
       if(key == 'Data criacão OS'){
        ...
        //console.log(colunas.NomedoProduto)
       
        
        // Ultima Coluna
        if(key == 'Tipo de Reincidência'){
            var tipo = 8 
            const opcoes = ['COLCHAO', 'COL ','COLC ','COL.','COLC.','COLCHOBOX','COLCHONETE','COLCH ','CJ ORTOBOM','CJ.BOX','CONJ ESPUMA ORTOBOM','COLCHOES','CONJUGADO ORTOBOM','CJTO BOX','MOLA','MOLAS','CONJUGADO SL ESPUMA','CJ.BOX','COLKAPPES'];

            opcoes.forEach(palavra => {
              if (colunas.NomedoProduto.indexOf(palavra) !== -1) {
    ...
    fs.writeFileSync('dadosmoveis.json', JSON.stringify(dados).trim());
        console.log(dados);
  ...
  async enviaOs(){
    ...
    //return;
    var content = JSON.parse(fs.readFileSync("dadosmoveis.json"));
...
const teste = new importacaomoveis();
 teste.importaZurich();
