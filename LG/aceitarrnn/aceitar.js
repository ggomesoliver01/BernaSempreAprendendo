  ...
  (async () => {
    for (let loop = 0; loop < arrdados.length; loop++) {
     ...
     if (arrdados[i].rnn != '' && arrdados[i].rnn != undefined){
      ...
      const iwecow = await page.$eval(
        "#MainFrame_mainVframeset_workVframeset_mdiFrameset_MENU00000_form_divMain_divWork_divServiceRepair_edtCicWarrantyFlag_input",
        ({ value }) => value
      );
     ...
     if (iwecow === 'OW') {
      user = 91;
  } else if (iwecow === 'IW' || iwecow === 'EP') {
      user = 90;
  } else if (iwecow === 'EC') {
      user = 147;
  } else {
      // Se o produto não corresponder a nenhum dos casos anteriores
