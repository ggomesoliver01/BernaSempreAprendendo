        ...
        try{
          ...
          if(aviso == "Senha de atendimento não confere ou não esta aberta."){
              ...
              if(nota.indexOf("Prezados, estamos verificando a possibilidade de reparo") !== -1) {
                  ...
                  await page.type('#formularioMenu\\:panelComment_body > table > tbody > tr:nth-child(1) > td:nth-child(1) > textarea',"Prezados, estamos verificando a possibilidade de reparo do produto e verificando se o segurado irá aceitar o reparo.")
                  await rep.updateGatilhoSinistros(arr[i].id);
  ...
  async carregaOS(){
    var response = await axios
        .post('https://gsplanaltec.com/consultaBot/', {
          sqlQuery:   "SELECT IZ.id,IZ.OS,IZ.Sinistro,IZ.data,S.sel_nome  FROM importados_zurich IZ left join selects S ON S.sel_id = IZ.status  WHERE IZ.id_emp IN (89) AND gatilho = 0 AND status = 25  AND (STR_TO_DATE(IZ.data,'%d/%m/%Y') >= '2023-04-17' AND STR_TO_DATE(IZ.data,'%d/%m/%Y') <= '2030-12-30')  group by IZ.id;"
